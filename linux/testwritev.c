...
int main(int argc, char* argv[])
{
	...
	if (connfd < 0) {
		...
		if (stat(file_name, &file_stat) < 0) {	/* 目标文件不存在 */
			...
			if(S_ISDIR(file_stat.st_mode)){
				...
				/* 动态分配缓存区 file_buf，并指定其大小为目标文件的大小
				   file_stat.st_size 加 1，然后将目标文件读入缓存区 file_buf 中 */
				int fd = open(file_name, O_RDONLY);
				file_buf = (char *)malloc(file_stat.st_size + 1);//new char [file_stat.st_size + 1];
		...
		/* 如果目标文件夹有效，则发送正常的 HTTP 应答 */
		if (valid) {
			/* 下面这部分内容将 HTTP 应答的状态行、"Content-Length" 头部字段和一个空行一次加入 header_buf 中 */
